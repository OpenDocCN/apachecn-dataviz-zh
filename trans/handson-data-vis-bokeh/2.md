...
字形是 Bokeh 的基本构件，用于创建各种各样的地块。事实上，您在使用 Bokeh 时构建的每个单独的绘图都有一个附加的字形机制，例如线条、矩形、圆形和组成绘图的任何其他对象。
...
当你看到一个情节，不管是互动的还是非互动的，它通常是由构成情节中每一个元素的几何形状组成的。在博克，这些几何形状被称为字形。如果我们想要创建一个线图，我们必须使用**线**来表示图上的信息，如果我们想要创建一个以圆为标记的散点图，我们将使用**圆**来表示该信息。
...
现在让我们看一下构建一个简单的酒吧情节。我们可以使用以下代码来实现这一点:
...
这一章已经给你介绍了什么是字形，以及如何使用它们来创建使用博克的基本图。我们还研究了如何进一步定制这些图。

字形是 Bokeh 的基本构造块，为了在未来创建更复杂、更有统计意义的地块，需要字形。
