...
![How to do it...](graphics/3367OS_04_02.jpg)
...
如果你从一开始就阅读这本书，你可能对`subplot`类很熟悉，它是`axes`的后代，生活在`subplot`实例的规则网格上。我们将解释并演示如何以高级方式使用支线剧情。

在这个食谱中，我们将学习如何在我们的剧情中创建自定义的子剧情配置。

## 做好准备

支线剧情的基类是`matplotlib.axes.SubplotBase`。这些子情节是`matplotlib.axes.Axes`实例，但提供了在图形中生成和操纵一组`Axes`的辅助方法。
...
`matplotlib.pyplot.subplots`用于轻松创建支线剧情的通用布局。我们可以指定网格的大小——子图网格的行数和列数。

我们可以创建共享 x 轴或 y 轴的子场景。这是使用`sharex`或`sharey`关键字参数实现的。参数`sharex`可以有值`True`，在这种情况下，x 轴在所有子剧情中共享。除了最后一行图，刻度标签在所有图上都不可见。也可以定义为字符串，枚举值为`row`、`col`、`all`或`none`。数值`all`与`True`相同，数值`none`与`False`相同。如果指定了值`row`，则每个子图行共享 x 轴。如果指定了值`col`，则每个子图列共享 x 轴。该助手返回元组`fig, ax`，其中`ax`或者是轴实例，或者，如果创建了多个子图，则是轴实例的数组。

`matplotlib.pyplot.subplots_adjust`用于调整支线剧情布局。关键字参数指定图形内的支线剧情坐标(`left`、`right`、`bottom`和`top`)归一化为图形大小。分别使用宽度和高度的`wspace`和`hspace`参数，可以指定在子情节之间留出空白。

## 怎么做...

1.  我们将向您展示在 matplotlib 工具包中使用另一个助手函数的示例— `subplot2grid`。我们定义网格的几何形状和子图的位置。请注意，这个位置是从 0 开始的，而不是像我们在`plot.subplot()`中习惯的从 1 开始。我们还可以使用`colspan`和`rowspan`来允许子剧情跨越给定网格中的多列和多行。例如，我们将:创建一个图形；使用`subplot2grid`添加各种支线剧情布局；重新配置刻度标签大小。
2.  Show the plot:
    ...
    ![How to do it...](graphics/3367OS_04_01.jpg)
...
![Getting ready](graphics/3367OS_04_03.jpg)
...
![Getting ready](graphics/3367OS_04_04.jpg)
...
![How to do it...](graphics/3367OS_04_05.jpg)
...
等高线图的另一个风险是选择绘制等值线的数量。如果我们选择太多，情节会变得太密集而无法解码，如果我们选择太少的等值线，我们会丢失信息，并且可以以不同的方式感知数据。
...
![How to do it...](graphics/3367OS_04_06.jpg)
...
![How to do it...](graphics/3367OS_04_10.jpg)
...
![How to do it...](graphics/3367OS_04_11.jpg)
...
![How to do it...](graphics/3367OS_04_12.jpg)
...
![How to do it...](graphics/3367OS_04_13.jpg)
