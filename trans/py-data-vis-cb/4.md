...
支线剧情的基类是`matplotlib.axes.SubplotBase`。这些子绘图是`matplotlib.axes.Axes`实例，但提供了在图形中生成和操纵一组`Axes`的辅助方法。
...
`matplotlib.pyplot.subplots_adjust`用于调整支线剧情布局。关键字参数指定图形内的支线剧情坐标(`left`、`right`、`bottom`和`top`)归一化为图形大小。分别使用宽度和高度的`wspace`和`hspace`参数，可以指定在子绘图之间留出空白。
