...
matplotlib 是一个强大的工具箱，它几乎满足了我们对 2D 的所有需求，也满足了一些 3D 绘图需求。作者打算让你学习 matplotlib 的最好方法是通过例子。当我们需要画一个绘图时，我们会寻找一个类似的例子，并试图改变它以适应我们的需要。这样，我们也准备给大家呈现一些有用的例子，相信这个例子会帮助大家找到一个最符合自己需要的剧情。
...
如果我们想要更多的控制，我们需要使用`matplotlib.axes.Axes`类的轴实例。它们允许我们在图中的任何位置放置地块。一个例子是把一个小的绘图放在一个大的绘图里。
...
图例和注释在上下文中清晰地解释了数据图。通过给每个绘图分配一个关于它所代表的数据的简短描述，我们在读者(观众)的头脑中启用了一个更简单的心智模型。这个食谱将展示如何在我们的图形上标注特定的点，以及如何创建和定位数据图例。
