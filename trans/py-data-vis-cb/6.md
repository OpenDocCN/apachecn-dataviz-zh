...
此外，我们将展示如何使用图像文件作为 matplotlibs 图表的注释。
...
我们将使用 Python matplotlib 库的功能，使用带有高级位置设置的图像和文本进行注释，以及箭头功能。
...
在我们定义了辅助函数之后，我们可以接近图形对象的构造并添加子场景。我们将在每年的年份集合中使用船的图像对这些进行注释，将图像缩放到适当的大小:
...
![How to do it...](graphics/3367OS_06_01.jpg)
...
`AnnotationBbox`是一个类似注释的类，但是它可以显示其他`OffsetBox`实例，而不是像`Axes.annotate`函数那样只显示文本。这允许我们在注释中加载图像或文本对象，并将其定位在离数据点特定距离的位置，以及使用箭头功能(`arrowprops`)来精确指向注释的数据点。

我们为`AnnotateBbox`构造函数提供了某些参数:

*   `Imagebox`:这一定是`OffsetBox`的一个实例(比如`OffsetImage`)；它是注释框的内容
*   `xy`:这是标注涉及的数据点坐标
*   `xybox`:定义标注框的位置
...
我们使用来自`pirates`列表的相同数据项，以稍微不同的相对位置向该图添加文本注释。第二个`AnnotationBbox`的大部分论点都是一样的——我们调整`xybox`和`pad`以将文本定位到行的对面。文本在`TextArea`类实例中，这类似于我们对图像所做的，但是文本`time.TextArea`和`OffsetImage`继承自同一个父类`OffsetBox`。
...
![How it works...](graphics/3367OS_06_02.jpg)
...
![How to do it...](graphics/3367OS_06_03.jpg)
...
![How to do it...](graphics/3367OS_06_04.jpg)
...
![How to do it...](graphics/3367OS_06_05.jpg)
