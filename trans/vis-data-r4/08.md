...
审美功能影响情节的内容。有四个函数可以设置美学–AES()、aes_()、aes_q()和 aes_string()。aes_()、aes_q()和 aes_string()函数共享一个帮助页面，并被描述为引用美学变量，以使变量编程更容易。我们这里只讲 aes()。有关这三个函数的更多信息，请参见其他函数的帮助页面。
