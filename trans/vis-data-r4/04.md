...
contour()函数绘制等高线图。该功能可以作为独立功能或辅助功能运行。在本章中，我们仅将该功能作为辅助功能。本例中的数据集是数据集包中的火山数据集。有关数据集的描述，请参见 4.2.4.2 部分，或者参见火山的帮助页面
...
![../images/502384_1_En_4_Chapter/502384_1_En_4_Fig20_HTML.jpg](../images/502384_1_En_4_Chapter/502384_1_En_4_Fig20_HTML.jpg)
...
## 4.4 提供关于某个图的信息或与该图进行交互的功能

本节介绍图例()、多行文字()、标识()和定位器()函数。legend()和 mtext()函数向地块添加图例和边距文本。identify()和 locator()函数使用计算机的指针与绘图交互。

### 4.4.1 图例( )和多行文字( )功能

当在绘图中使用不同的绘图字符、不同颜色或样式的线条或不同颜色或样式的阴影区时，通常使用图例。legend()函数在绘图上放置一个图例。有时一个来源，信用，或其他信息应该被包括在一个情节，在页边空白处。函数的作用是:将文本放在空白处。
...
trace 参数采用任意长度的逻辑向量(或者可以强制为逻辑向量)。如果跟踪的长度大于 1，则会给出警告，并且只使用第一个元素。然而，传说情节。如果 trace 设置为 TRUE，将在 R 控制台上打印一些诊断信息。如果为 FALSE，则不会在 R 控制台上打印任何内容。
...
horiz 参数采用任意长度的逻辑向量(或者可以强制为逻辑向量)。如果长度大于 1，则会给出警告，并且只使用第一个元素。然而，传说情节。
...
![../images/502384_1_En_4_Chapter/502384_1_En_4_Fig21_HTML.jpg](../images/502384_1_En_4_Chapter/502384_1_En_4_Fig21_HTML.jpg)
...
![../images/502384_1_En_4_Chapter/502384_1_En_4_Fig23_HTML.jpg](../images/502384_1_En_4_Chapter/502384_1_En_4_Fig23_HTML.jpg)

图 4-23

清单结果 [4-24](#PC24) ，运行 identify()的例子

![../images/502384_1_En_4_Chapter/502384_1_En_4_Fig22_HTML.jpg](../images/502384_1_En_4_Chapter/502384_1_En_4_Fig22_HTML.jpg)
...
这些函数在运行时会等待用户点击指针指定的次数。要在 Quartz 中给定次数之前退出任一功能，请按 Esc 键。对于 X11 或 Windows，使用第一个按钮以外的任何按钮单击鼠标。根据 identify()和 locator()的帮助页面，只有装有 X11、Quartz 或 Windows 的电脑才能使用这些功能。
...
容差参数采用长度应为 1 的数值向量。如果更长，则不会给出警告，而是只使用第一个值。该参数给出了围绕点的圆的半径(以英寸为单位),在圆内单击该半径会告诉 identify()识别并(可能)标记该点。如果指针被单击的位置附近没有足够的点，则会发出警告，但功能会继续执行-如果存在具有重叠圆的点，情况也是如此。公差的默认值为 0.25。
...
![../images/502384_1_En_4_Chapter/502384_1_En_4_Fig24_HTML.jpg](../images/502384_1_En_4_Chapter/502384_1_En_4_Fig24_HTML.jpg)
