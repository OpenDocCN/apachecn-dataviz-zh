...
![Figure 4.1: Seaborn Relation plot](image/C12815_04_01.jpg)

###### 图 4.1:海底关系图

幕后，西博恩用 Matplotlib 绘制剧情。尽管许多任务只需使用 Seaborn 就可以完成，但进一步的定制可能需要使用 Matplotlib。我们只提供了数据集中变量的名称以及它们在图中扮演的角色。与 Matplotlib 不同，没有必要将变量转换为可视化的参数。
...
正如我们之前提到的，Matplotlib 是高度可定制的。但这也有一个影响，那就是很难知道要调整什么样的设置来实现视觉上吸引人的情节。相比之下，Seaborn 提供了几个定制的主题和一个高级界面来控制 Matplotlib 图形的外观。
...
![Figure 4.2: Matplotlib line plot](image/C12815_04_02.jpg)
...
![Figure 4.3: Seaborn line plot](image/C12815_04_03.jpg)

###### 图 4.3:海伯恩线图

Seaborn 将 Matplotlib 的参数分为两组。第一组包含剧情美学的参数，而第二组缩放图形的各种元素，以便可以在不同的上下文中轻松使用，例如用于演示、海报等的可视化。

### 海鸟体形风格

为了控制风格，Seaborn 提供了两种方法:`set_style(style, [rc])`和`axes_style(style, [rc])`。

`seaborn.set_style(style, [rc])`设定剧情的审美风格。
...
![Figure 4.4: Seaborn line plot with whitegrid style](image/C12815_04_04.jpg)

###### 图 4.4:白色网格样式的海伯恩线图

`seaborn.axes_style(style, [rc])`返回剧情审美风格的参数字典。该函数可在 with 语句中用于临时更改样式参数。
...
![Figure 4.5: Seaborn Line plot with dark axes style](image/C12815_04_05.jpg)
...
`seaborn.despine(fig=None, ax=None, top=True, right=True, left=False, bottom=False, offset=None, trim=False)`从剧情中移除顶部和右侧的刺。
...
![Figure 4.6: Despined Seaborn line plot](image/C12815_04_06.jpg)

###### 图 4.6:去毛刺的海底线图

### 上下文

一组单独的参数控制地块元素的比例。这是一种使用相同代码创建适合在需要更大或更小图的环境中使用的图的简便方法。为了控制上下文，可以使用两个函数。

`seaborn.set_context(context, [font_scale], [rc])`设置绘图环境参数。这不会改变剧情的整体风格，但会影响标签、线条等的大小。基础上下文是`notebook`，其他上下文是`paper`、`talk`和`poster`，它们分别是按 0.8、1.3 和 1.6 缩放的`notebook`参数的版本。
...
![Figure 4.7: Seaborn line plot with poster context](image/C12815_04_07.jpg)
...
5.  使用`whitegrid`样式，将上下文设置为说话，去掉除底部的轴刺以外的所有轴刺。给情节加上一个标题。
6.  执行上述步骤后，最终输出应如下所示:

![Figure 4.8: IQ scores of groups](image/C12815_04_08.jpg)
...
您可以使用`set_palette()`设置所有地块的调色板。该函数接受与`color_palette()`相同的参数。在接下来的章节中，我们将解释调色板如何被分成不同的组。
...
![Figure 4.14: Colorblind color palette ](image/C12815_04_14.jpg)
...
![Figure 4.15: Custom brown color palette ](image/C12815_04_15.jpg)
...
![Figure 4.16: Custom reversed brown color palette ](image/C12815_04_16.jpg)
...
![Figure 4.17: Coolwarm color palette ](image/C12815_04_17.jpg)
...
![Figure 4.18: Custom diverging color palette ](image/C12815_04_18.jpg)
...
![Figure 4.19: Heatmap of Flight Passengers data ](image/C12815_04_19.jpg)
...
在上一章中，我们讨论了 Matplotlib 中的各种情节，但仍有一些可视化的内容需要讨论。
...
![Figure 4.20: Seaborn bar plot ](image/C12815_04_20.jpg)
...
![Figure 4.21: Movie Scores comparison ](image/C12815_04_21.jpg)
...
![Figure 4.22: KDE with histogram for univariate distribution ](image/C12815_04_22.jpg)
...
![Figure 4.23: KDE for univariate distribution ](image/C12815_04_23.jpg)
...
![Figure 4.28: Scatter plot with marginal histograms](image/C12815_04_24.jpg)
...
![Figure 4.25: Contour plot ](image/C12815_04_25.jpg)
...
![Figure 4.26: Seaborn pair plot ](image/C12815_04_26.jpg)
...
![Figure 4.31: Seaborn violin plot](image/C12815_04_27.jpg)
...
![Figure 4.28: Violin plot showing IQ scores of different groups ](image/C12815_04_28.jpg)
...
在前一个主题中，我们介绍了一个多情节，即配对情节。在这个话题中，我们想谈谈一种不同的方式来创造灵活的多情节。
...
![Figure 4.29: FacetGrid with scatter plots ](image/C12815_04_29.jpg)
...
![Figure 4.30: Subscribers and Views of top 30 YouTube channels ](image/C12815_04_30.jpg)
...
![Figure 4.31: Seaborn regression plot ](image/C12815_04_31.jpg)
...
![Figure 4.32: Linear regression for animal attribute relations ](image/C12815_04_32.jpg)
...
![Figure 4.44: Tree map](image/C12815_04_33.jpg)
...
![Figure 4.34: Tree map of water usage ](image/C12815_04_34.jpg)
