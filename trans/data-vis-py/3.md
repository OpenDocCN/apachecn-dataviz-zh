...
在我们开始使用 Matplotlib 创建我们的第一个可视化之前，我们将理解并尝试掌握这些情节背后的概念。

## Matplotlib 中的地块概述

**Matplotlib 中的地块**具有层次结构，嵌套 Python 对象以创建树状结构。每个图都封装在一个`Figure`对象中。这个`Figure`是可视化的顶层容器。它可以有多个轴，这些轴基本上是这个顶层容器中的独立图。
...
当我们看一个图形的“解剖”时，如下图所示，我们对一个有洞察力的可视化的复杂性有了一个概念。Matplotlib 通过调整**网格**、 **x** 和 **y** 刻度、**刻度标签**和**图例**，不仅让我们能够简单地显示数据，还可以围绕数据设计整个**图形**。这意味着我们可以修改情节的每一个细节，从标题和图例开始，一直到脊柱上的大刻度和小刻度，以使其更具表现力:

![Figure 3.1: Anatomy of a Matplotlib Figure](image/C12815_03_01.jpg)
...
![Figure 3.2 Color specified in string format ](image/C12815_03_02.jpg)

###### 图 3.2 以字符串格式指定的颜色

下图显示了所有可用的标记选项:

![Figure 3.3: Markers in format strings](image/C12815_03_03.jpg)

###### 图 3.3:格式字符串中的标记

下图显示了所有可用的线型:

![Figure 3.4: Line styles](image/C12815_03_04.jpg)
...
4.  Plot the following data pairs `(x, y)` as circles, which are connected via line segments: `(1, 1)`, `(2, 3)`, `(4, 4)`, `(5, 3)` . Then, visualize the plot:
    ...
    ![Figure 3.8: A simple visualization that was created with the help given data pairs connected via line segments](image/C12815_03_08.jpg)

    ###### 图 3.8:在给定数据对的帮助下创建的简单可视化，并通过线段连接

5.  使用`plt.savefig()`方法保存剧情。这里，我们可以在方法中提供一个文件名，或者指定完整的路径:
...
与放置在轴上任意位置的文本相比，**注释**用于注释地块的某些特征。在标注中，有两个位置需要考虑:标注的位置`xy`和标注的位置，文本`xytext`。指定参数`arrowprops`非常有用，它会产生一个指向注释位置的箭头。
...
这将创建一个指向数据坐标(4，2)的绿色箭头，文本“注释示例”位于数据坐标(8，4)处:

![Figure 3.9: Implementation of Text Commands](image/C12815_03_09.jpg)
...
![Figure 3.10: Legend example](image/C12815_03_10.jpg)
...
![Figure 3.11: Visualization of stock trends of five companies](image/C12815_03_11.jpg)
...
![Figure 3.12: A simple bar chart](image/C12815_03_12.jpg)
...
![Figure 3.13: Bar chart with subcategories](image/C12815_03_13.jpg)
...
3.  使用电影标题作为 x 轴的标签。对 y 轴刻度使用 20 的间隔百分比，对 5 的间隔使用次要刻度。给情节加上一个图例和合适的标题。
4.  执行上述步骤后，预期输出应该如下:

![Figure 3.14: Bar plot comparing scores of five movies](image/C12815_03_14.jpg)
...
![Figure 3.15: Basic pie chart](image/C12815_03_15.jpg)
...
![Figure 3.16: Pie chart for water usage](image/C12815_03_16.jpg)
...
![Figure 3.17: Stacked bar chart](image/C12815_03_17.jpg)
...
3.  After executing the preceding steps, the expected output should be as follows:
    ...
    ![Figure 3.18: Stacked bar chart showing performance of restaurant on different days](image/C12815_03_18.jpg)
...
![Figure 3.19: Stacked area chart](image/C12815_03_19.jpg)
...
![Figure 3.20: Stacked area chart comparing sales units of different smartphone manufacturers](image/C12815_03_20.jpg)
...
![Figure 3.21: Histogram](image/C12815_03_21.jpg)

###### 图 3.21:直方图

`plt.hist2d(x, y)`创建 2D 直方图。下图显示了 2D 历史图的一个示例:

![Figure 3.22: 2D histogram with color bar](image/C12815_03_22.jpg)
...
![Figure 3.23: Box plot](image/C12815_03_23.jpg)
...
4.  The expected output for step 1 is as follows:
    ...
    ![Figure 3.24: Histogram for IQ test](image/C12815_03_24.jpg)

    ###### 图 3.24:智商测试直方图

5.  The expected output for step 2 is as follows:

    ![Figure 3.25: Box plot for IQ scores](image/C12815_03_25.jpg)

    ###### 图 3.25:智商分数的方框图

6.  The expected output for step 3 is as follows:

    ![Figure 3.26: Box plot for IQ scores of different test groups](image/C12815_03_26.jpg)
...
![Figure 3.27: Scatter plot](image/C12815_03_27.jpg)
...
![Figure 3.28: Scatter plot on animal statistics](image/C12815_03_28.jpg)
...
![Figure 3.29: Bubble plot with color bar](image/C12815_03_29.jpg)

###### 图 3.29:带颜色条的气泡图

## 布局

在 Matplotlib 中定义可视化布局有多种方法。我们将从**支线剧情**开始，以及如何使用**紧凑布局**来创建视觉上吸引人的剧情，然后覆盖 **GridSpec** ，这为创建多剧情提供了更灵活的方式。

### 支线剧情

并排显示几个图通常很有用。Matplotlib 提供了子情节的概念，子情节是一个图形中的多个轴。这些图可以是网格图、嵌套图等。

探索以下选项来创建子情节:

*   `plt.subplots(nrows, ncols)`创建一个人物和一组支线剧情。
*   `plt.subplot(nrows, ncols, index)`或等效的`plt.subplot(pos)`给当前图形增加一个子图。指数从 1 开始。`plt.subplot(2, 2, 1)`相当于`plt.subplot(221)`。
*   `Figure.subplots(nrows, ncols)`向指定的图形添加一组子情节。
...
![Figure 3.30: Subplots](image/C12815_03_30.jpg)
...
![Figure 3.31: Subplots with a shared x and y axis](image/C12815_03_31.jpg)

###### 图 3.31:共享 x 轴和 y 轴的子情节

### 紧凑布局

`plt.tight_layout()`调整支线剧情参数，使支线剧情在图中很好的契合。

**示例:**

如果不使用`plt.tight_layout()`，支线剧情可能会重叠:
...
![Figure 3.32: Subplots with no layout option](image/C12815_03_32.jpg)

###### 图 3.32:没有布局选项的支线剧情

使用`plt.tight_layout()`不会导致子情节重叠:
...
![Figure 3.33: Subplots with a tight layout](image/C12815_03_33.jpg)
...
5.  用极坐标投影创建支线剧情。设置一个紧凑的布局，这样就不会有任何重叠:
...
![Figure 3.34: Radar charts](image/C12815_03_34.jpg)
...
![Figure 3.37: Grayscale image with default viridis colormap](image/C12815_03_37.jpg)

###### 图 3.37:默认维里迪斯色图的灰度图像

下图显示了带有灰色色图的灰度图像:

![Figure 3.38: Grayscale image with gray colormap](image/C12815_03_38.jpg)

###### 图 3.38:带有灰色色图的灰度图像

下图显示了一幅 RGB 图像:

![Figure 3.39: RGB image](image/C12815_03_39.jpg)
...
![Figure 3.40: Image with jet colormap and color bar](image/C12815_03_40.jpg)
...
![Figure 3.41: Histogram of image values](image/C12815_03_41.jpg)
...
![Figure 3.43: Multiple images with labels](image/C12815_03_43.jpg)
...
![](image/C12815_03_45.jpg)

###### 图 3.45:演示数学表达式的图

**TeX 示例:**

*   `'$\alpha_i>\beta_i$'`生产![](image/C12815_03_45a.png)
*   `'$\sum_{i=0}^\infty x_i$'`生产![](image/C12815_03_45b.png)
*   `'$\sqrt[3]{8}$'`生产![](image/C12815_03_45c.png)
*   `'$\frac{3 - \frac{x}{2}}{5}$'`生产![](image/C12815_03_45d.png)
